...
func _on_wizard_pressed() -> void:
	...
	if peer_id == -1:
		return
	var packet := ClassSelect.create(peer_id, EnumHandler.CLASSES.WIZARD)
...
func _on_barbarian_pressed() -> void:
	...
	if peer_id == -1:
		return
	var packet := ClassSelect.create(peer_id, EnumHandler.CLASSES.BARBARIAN)
	if NetworkHandler.server_peer:
		packet.send(NetworkHandler.server_peer)
...
func _on_rogue_pressed() -> void:
	...
	if peer_id == -1:
		return
	var packet := ClassSelect.create(peer_id, EnumHandler.CLASSES.ROGUE)
	if NetworkHandler.server_peer:
		packet.send(NetworkHandler.server_peer)
...
func _on_bard_pressed() -> void:
	...
	if peer_id == -1:
		return
	var packet := ClassSelect.create(peer_id, EnumHandler.CLASSES.BARD)
	if NetworkHandler.server_peer:
		packet.send(NetworkHandler.server_peer)
...
func _on_druid_pressed() -> void:
	...
	if peer_id == -1:
		return
	var packet := ClassSelect.create(peer_id, EnumHandler.CLASSES.DRUID)
	if NetworkHandler.server_peer:
		packet.send(NetworkHandler.server_peer)
